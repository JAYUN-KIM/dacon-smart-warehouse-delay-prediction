...
- 즉 backbone 쪽에 실제로 새로운 신호가 같이 들어와야 함

### future-window 해석은 실제 신호다

`a110`에서는 문제를 다시 창고 운영 관점에서 보고, 가까운 미래 슬롯의 부하와 압력 요약을 직접 피처로 넣었습니다.

- local direct router OOF: `7.4081`
- public: `10.106859334`

의미:
- 최고 public은 못 넘었지만 correction layer 계열과는 다르게 새로운 backbone 신호가 실제로 작동했음
- 향후 방향은 future-window 해석을 버리는 것이 아니라, `a101`의 강한 자산과 더 안정적으로 결합하는 쪽이 맞음
...
5. 동시에 future-window 기반의 창고 부하 해석을 backbone 쪽으로 더 키울 필요가 있다.
