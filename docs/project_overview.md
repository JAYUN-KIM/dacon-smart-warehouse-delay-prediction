...
- `a114`: 미래창을 scenario baseline/scale 강화용으로 넣어 실제 최고 public 달성
...
반대로 `a114`는 미래창 신호를 scenario baseline/scale에 집중해서 넣었고, 실제 public 최고 기록을 만들었습니다.
...
1. `a100 family`는 맞았다.
2. `a114_09`가 현재 가장 강한 public 앵커다.
3. correction layer를 더 얹는 방향은 당분간 우선순위를 낮춘다.
4. future-window 신호는 유지하되, scenario-level stress 해석에 집중한다.
5. `a115` 결과를 보면 이제 같은 family 안에서 후보 생성만 더 깎는 방식은 한계가 보인다.
6. 다음은 지금까지 얻은 해석을 살린 **새 direct family** 설계가 더 중요하다.

즉 지금은 `a114`를 그냥 더 미세하게 다듬는 단계가 아니라, `a114`가 왜 먹혔는지를 이용해 다음 backbone을 설계하는 단계입니다.
