...
- `a118`: `a117_09`의 안전한 분포 대역을 기준으로 positive/negative delta를 비대칭 클리핑한 내일 제출 후보 생성

이 계열의 핵심은 direct model을 그대로 믿지 않는 것입니다. 약한 direct prediction이라도 기존 앵커와 다른 방향의 delta를 제공할 수 있고, 이 delta를 낮은 alpha와 clipping으로 제한할 때 실제 public 개선이 나왔습니다.
...
5. `a116` ridge seq2seq direct model은 단독으로 약하지만 delta provider로 가치가 있다.
6. 다음은 `a118` public 결과를 보고 asymmetric clipping / alpha / distribution guard를 더 좁히는 것이다.

즉 지금은 `a117_09`를 기준 앵커로 두고, public에서 실제로 먹힌 clipped delta band를 더 정밀하게 찾는 단계입니다.
